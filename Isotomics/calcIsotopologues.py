...
setsOfElementIsotopes = {'H':(0,1),'N':(0,1),'C':(0,1),'O':(0,1,2),'S':(0,1,2,4),'Cl':(0,2)}
...
def condenseStr(text):
    '''
    Takes the "expanded" string depictions, i.e. "(0,1)0" for multiatomic sites and transforms them into "ATOM" depictions, i.e. "010". This makes it easy to pick out the element for a particular substitution by finding the index of the ATOM depiction and looking at that same index in listSiteElements. 
...
def listSiteElements(molecularDataFrame):
    '''
    Our dataframe may include multiatomic sites--for example, we may define site N1/N2 to include two nitrogens and site O3 to have one oxygen. It is useful to have a list where we can index in by position--i.e. ['Cl','C','H']--to determine the chemical element at a given position. This function defines that list. 
    
    Inputs:
        molecularDataFrame: A dataFrame containing information about the molecule.
        
    Outputs: 
        siteElements: A list giving the chemical element by position, expanding multiatomic sites. 
    '''
    elIDs = molecularDataFrame['IDS'].values
    numberAtSite = molecularDataFrame['Number'].values
    ...
    siteElementList = []
    for site in siteList:
        siteElementList += [site[0]] * site[1]
    
    return siteElementList
...
def calcAtomDictionary(isotopologueConcentrationDict, molecularDataFrame, disable = False):
    ...
    '''
    siteElements = listSiteElements(molecularDataFrame)
